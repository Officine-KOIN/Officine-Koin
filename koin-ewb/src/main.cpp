...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Koin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Koin Signed Message:\n";
...
static const int64 nTargetTimespan = 12 * 60 * 60; // Koin: every 12 hours
static const int64 nTargetSpacing = 90; // Koin: 1 minute, 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Koin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal koin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Koin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Koind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
